...
import com.cnc.qr.api.csdvorder.resource.GetDeliveryTypeFlagInputResource;
import com.cnc.qr.api.csdvorder.resource.GetDeliveryTypeFlagOutputResource;
import com.cnc.qr.api.csdvorder.resource.GetStoreAdverPicInputResource;
import com.cnc.qr.api.csdvorder.resource.GetStoreAdverPicOutputResource;
import com.cnc.qr.api.csdvorder.resource.GetStoreInfoInputResource;
import com.cnc.qr.api.csdvorder.resource.GetStoreInfoOutputResource;
import com.cnc.qr.api.csdvorder.resource.GetStoreLanguagesInputResource;
import com.cnc.qr.api.csdvorder.resource.GetStoreLanguagesOutputResource;
import com.cnc.qr.api.csdvorder.resource.GetStorePaymentInfoInputResource;
import com.cnc.qr.api.csdvorder.resource.GetStorePaymentInfoOutputResource;
import com.cnc.qr.api.csdvorder.resource.GetTableInputResource;
import com.cnc.qr.api.csdvorder.resource.GetTableOutputResource;
 ...
 */
@RestController
public class CsdvStoreInfoController {
     ...
     * @return 店舗情報
     */
    @PostMapping(UrlConstants.CSDV_GET_STORE_INFO)
     ...
     * @return 店舗媒体情報
     */
    @PostMapping(UrlConstants.CSDV_GET_STORE_ADVERPIC)
    @ResponseStatus(HttpStatus.OK)
    @PreAuthorize("hasRole(\"" + AuthoritiesConstants.MOBILE + "\")")
     ...
     * @return 店舗言語情報
     */
    @PostMapping(UrlConstants.CSDV_GET_ACTIVE_LANGUAGE)
    @ResponseStatus(HttpStatus.OK)
    @PreAuthorize("hasRole(\"" + AuthoritiesConstants.MOBILE + "\")")
     ...
     * @return 店舗支払方式情報
     */
    @PostMapping(UrlConstants.CSDV_GET_STORE_PAYMENT_INFO)
    @ResponseStatus(HttpStatus.OK)
    @PreAuthorize("hasRole(\"" + AuthoritiesConstants.MOBILE + "\")")
     ...
     * @return テーブル情報取得
     */
    @PostMapping(UrlConstants.CSDV_GET_TABLE_URL)
    @ResponseStatus(HttpStatus.OK)
    @PreAuthorize("hasRole(\"" + AuthoritiesConstants.MOBILE + "\")")
     ...
     * @return 出前仕方フラグ取得
     */
    @PostMapping(UrlConstants.CSDV_GET_DELIVERY_TYPE_FLAG)
    @ResponseStatus(HttpStatus.OK)
    @PreAuthorize("hasRole(\"" + AuthoritiesConstants.MOBILE + "\")")
    public GetDeliveryTypeFlagOutputResource getDeliveryTypeFlag(
        @Validated @RequestBody GetDeliveryTypeFlagInputResource inputResource,
        BindingResult result) {
        ...
        // インプット情報をDTOにセットする
        GetDeliveryTypeFlagInputDto inputDto = beanMapper
            .map(inputResource, GetDeliveryTypeFlagInputDto.class);

        // テーブルサービス処理を実行する
        GetDeliveryTypeFlagOutputDto outputDto = storeInfoService.getDeliveryTypeFlag(inputDto);

        return beanMapper.map(outputDto, GetDeliveryTypeFlagOutputResource.class);
    }
}
