...
import com.cnc.qr.api.csdvorder.resource.TaxInfoInputResource;
import com.cnc.qr.api.csdvorder.resource.TaxInfoOutputResource;
import com.cnc.qr.api.csdvorder.resource.TaxValueInputResource;
import com.cnc.qr.api.csdvorder.resource.TaxValueOutputResource;
 ...
 */
@RestController
public class CsdvTaxInfoController {
     ...
     * @return 税金额
     */
    @PostMapping(UrlConstants.CSDV_GET_TAX_VALUE)
    ...
    public TaxValueOutputResource getTaxValue(
        ...
        if (null != inputResource.getSelectDeliveryType()
            && DeliveryTypeFlag.CATERING.getCode().equals(inputResource.getSelectDeliveryType())) {

            BigDecimal taxTotal = inputResource.getTotalPrice().add(outputDto.getSotoTaxEight())
                .add(outputDto.getSotoTaxTen());

            cateringCharge = taxInfoService.getCateringCharge(inputDto.getStoreId(), taxTotal);

        }

        // インプット情報をDTOにセットする
        TaxValueOutputResource outputResource = beanMapper
            .map(outputDto, TaxValueOutputResource.class);

        outputResource.setCateringCharge(cateringCharge);
     ...
     * @return 税情報
     */
    @PostMapping(UrlConstants.CSDV_GET_TAX_INFO)
    @ResponseStatus(HttpStatus.OK)
    @PreAuthorize("hasRole(\"" + AuthoritiesConstants.MOBILE + "\")")
